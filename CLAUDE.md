...
8. **Все секционные компоненты должны иметь одинаковые вертикальные отступы.** Добавлять классы `my-7.5 md:my-15` к элементу `<section>` для единообразия spacing между секциями на всех страницах.
