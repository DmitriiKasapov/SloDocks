...
<div class="{{ $class }}">
    ...
    <div class="grid sm:grid-cols-2 gap-4">
        @foreach($content['files'] ?? [] as $fileIndex => $file)
            @php
            ...
            <a href="{{ $downloadUrl }}"
               class="flex items-center gap-4 p-5 bg-white rounded-xl shadow-sm border border-gray-100 hover:shadow-md hover:border-indigo-300 transition-all group">
                <div class="shrink-0 w-12 h-12 gradient-icon-red rounded-lg flex items-center justify-center">
                ...
                <svg class="w-5 h-5 text-gray-400 group-hover:text-indigo-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
