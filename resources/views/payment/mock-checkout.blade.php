    ...
    <div class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="px-6 pb-6 space-y-3">
            <form action="{{ route('payment.mock.pay', $purchase) }}" method="POST">
                @csrf
                <button type="submit"
                        class="w-full py-3 px-4 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition-colors">
                    Оплатить (тест)
                </button>
            </form>

            <a href="{{ route('payment.cancel') }}"
               class="block w-full py-3 px-4 text-center border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors">
                Отменить
            </a>
        </div>
